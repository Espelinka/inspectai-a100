...
import { PlusIcon, ArrowPathIcon, CheckCircleIcon, ListBulletIcon, CalendarDaysIcon, ChevronLeftIcon, PhotoIcon } from '@heroicons/react/24/solid';
...
export default function App() {
  ...
  const handleImageSelected = async (file: File) => {
    ...
    try {
      ...
      // Inject the local image URL into the response so it travels with the data
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newCard: ApartmentCard = { 
        ...
        // Ensure the photo URL is saved
        act_photos: processingResponse.apartment_card.act_photos || (currentImageUrl ? [{ filename: 'upload.jpg', url: currentImageUrl, confidence: 1 }] : [])
      };
